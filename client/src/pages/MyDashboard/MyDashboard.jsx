...
const MyDashboard = ({ onLogout }) => {
    ...
    useEffect(() => {
        ...
        if (storedUser) {
            ...
            setLogsLoading(true);
            getAccessLogs()
                .then(result => setAccessLogs(result.data || []))
                .catch(() => setAccessLogs([]))
        ...
        <div className="dashboard-page">
            ...
            <main className="dashboard-main">
                <div className="dashboard-container">
                    ...
                    <div className="dashboard-grid">
                        {activeMemberId ? (
                            <a href="/settings" className="dashboard-card">
                                ...
                                <h3 className="dashboard-card-title">{activeMemberName}'s Settings</h3>
                                <p className="dashboard-card-description">Profile and health information</p>
                            </a>
                        ) : (
                            <a href="/settings" className="dashboard-card">
                                <div className="dashboard-card-icon">
                    ...
                    {accessLogs.length > 0 && (
                        <div className="dashboard-access-logs">
                            <h2 className="dashboard-access-logs-title">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                                    <circle cx="12" cy="12" r="3"/>
                                </svg>
                                Recent Record Access Activity
                            </h2>
                            <div className="dashboard-access-logs-list">
                                {accessLogs.slice(0, 10).map((log, index) => (
                                    <div key={index} className="dashboard-access-log-item">
                                        <div className="dashboard-access-log-who">
                                            <strong>{log.recipientName || log.recipientEmail || log.accessedBy}</strong>
                                            {log.dataAccessed?.includes('pdf_download') && (
                                                <span className="dashboard-access-log-badge-download">PDF Downloaded</span>
                                            )}
                                        </div>
                                        <div className="dashboard-access-log-details">
                                            <span className="dashboard-access-log-date">
                                                {new Date(log.accessedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}
                                            </span>
                                            {log.dataAccessed && !log.dataAccessed.includes('pdf_download') && (
                                                <span className="dashboard-access-log-data">Viewed: {log.dataAccessed.join(', ')}</span>
                                            )}
                                        </div>
                                    </div>
                                ))}
