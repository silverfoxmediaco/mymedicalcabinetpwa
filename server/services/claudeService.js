...
- INSURANCE AWARENESS: Read the bill carefully for insurance payment information. Extract the total amount the insurance paid, any adjustments/discounts applied, and the remaining patient balance. The patientBalance field MUST reflect what the bill says the patient owes (total billed minus insurance paid minus adjustments). The recommendedPatientOffer MUST be based on what the PATIENT actually owes, NOT the total bill. Follow this logic strictly:
  1. If the bill shows a patient balance/amount due, use that as the starting point.
  2. Compare that patient balance against the fair price total minus insurance paid: fairPatientShare = max(0, fairPriceTotal - insurancePaid - adjustments).
  3. recommendedPatientOffer = min(patientBalance, fairPatientShare). The offer should NEVER exceed what the patient currently owes.
  4. If insurance paid more than the fair price total, the recommended offer should be $0 or very minimal — the insurance already covered fair value.
